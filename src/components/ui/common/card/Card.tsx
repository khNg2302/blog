...
export const Card = ({ children, className }: CardI) => {
  ...
  return isPortrait ? (
    <DisplayColumnItem className={cn("w-full gap-4 h-full", className)}>
      {children({ isPortrait })}
    </DisplayColumnItem>
  ) : (
    <DisplayCenterItem className="w-full h-full relative">
      {" "}
      {children({ isPortrait })}{" "}
    </DisplayCenterItem>
  );
...
const CardContent = ({
  ...
  return isPortrait ? (
    <DisplayColumnItem className={cn("flex-1 gap-2", className)}>
      {children}
    </DisplayColumnItem>
  ) : (
    <DisplayFullScreenMedia className={cn(className)}>
...
const CardFooter = ({ children }: ElementType<unknown>) => {
  return <DisplayColumnItem>{children}</DisplayColumnItem>;
};
