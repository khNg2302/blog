...
export const PrimaryAppButton = ({ children, ...props }: AppButtonI) => {
  return (
    <AppButton {...props} className="bg-sky-700" variant="default">
      {children}
    </AppButton>
  );
};
...
export const PrimaryIconAppButton = ({
  children,
  className,
  ...props
}: AppButtonI) => {
...
export const GhostIconWithUnderTextAppButton = ({
  children,
  className,
  ...props
}: AppButtonI) => {
  return (
    <GhostAppButton {...props} className={cn("!h-fit flex-1", className)}>
